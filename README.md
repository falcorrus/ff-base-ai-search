...
Если при запуске сервера возникают проблемы с подключением, попробуйте один из следующих вариантов:

1. Запуск сервера напрямую с помощью Python:
   ```bash
   # Активация виртуального окружения
   source backend/venv/bin/activate
   ...
   # Запуск сервера напрямую с помощью Python
   python -m uvicorn main:app --host 0.0.0.0 --port 8000
   ```

2. Использование специального скрипта для прямого запуска:
   ```bash
   cd backend
   ./start_direct.sh
   ```
