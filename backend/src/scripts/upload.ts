// backend/src/scripts/upload.ts
import { createClient } from '@supabase/supabase-js';
import * as fs from 'fs';
import * as path from 'path';
import dotenv from 'dotenv';

// Load environment variables
dotenv.config({ path: path.resolve(__dirname, '../../.env') });

// Read the JSON file synchronously
const basePath = path.resolve(__dirname, '../../../input_base.json');
console.log('Looking for input_base.json at:', basePath);
const rawData = fs.readFileSync(basePath, 'utf-8');
const data = JSON.parse(rawData);
console.log('Loaded', data.length, 'records from input_base.json');

const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;
const supabaseKey = process.env.SUPABASE_SERVICE_ROLE_KEY!; // ВАЖНО: используй service role key
const supabase = createClient(supabaseUrl, supabaseKey);

// Function to convert 768-dimensional embedding to 1536-dimensional embedding
function convertEmbedding(embedding: number[]): number[] {
  // If already 1536-dimensional, return as is
  if (embedding.length === 1536) {
    return embedding;
  }
  
  // If 768-dimensional, pad with zeros to make it 1536-dimensional
  if (embedding.length === 768) {
    return [...embedding, ...Array(768).fill(0)];
  }
  
  // For any other size, pad or truncate to 1536
  const result = new Array(1536).fill(0);
  const copyLength = Math.min(embedding.length, 1536);
  for (let i = 0; i < copyLength; i++) {
    result[i] = embedding[i];
  }
  return result;
}

async function upload() {
  console.log('Starting upload process...');
  const batchSize = 1000;
  for (let i = 0; i < data.length; i += batchSize) {
    // Convert embeddings for each document in the batch
    const batch = data.slice(i, i + batchSize).map((doc: any) => ({
      ...doc,
      embedding: convertEmbedding(doc.embedding)
    }));
    
    console.log(`Processing batch ${Math.floor(i/batchSize) + 1}: ${batch.length} records`);
    
    // Use upsert to handle existing records
    const { error } = await supabase.from('documents').upsert(batch, {
      onConflict: 'id'
    });
    if (error) {
      console.error('Ошибка загрузки:', error);
    } else {
      console.log(`Загружено/обновлено ${Math.min(i + batchSize, data.length)} из ${data.length}`);
    }
  }
  console.log('Upload process completed');
}

upload();